...
cls
color 07
title  Microsoft Activacne skripty AIO / Prelozil MoonLiian
...
%nceline%
echo NePodporovane OS Nájdene
echo Projekt Podporuje 7/8/8.1/10/11 a serverové ekvalenty.
goto MASend
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo Nemozem najst PowerShell.exe v systeme
echo Prerušenie...
goto MASend
)
...
%nceline%
echo Scenár sa spúša z doèasnej zložky,
echo Pravdepodobne spúšate scenár priamo z archívu.
echo:
echo Vytiahnite archívny súbor a spustite skript z vytiahnutej zložky.
goto MASend
)
)
...
%nceline%
echo Tento skript vyžaduje práva správcu.
echo Na tento ucel kliknite pravým tlacitkom na tento skript a Otvorit Za Spravcu
)
...
cls
color 07
title  Moonsoft Aktivacne Scripty AIO 1.7
mode 76, 30
...
echo:
echo:                 Aktivacne Metody:
echo:
echo:             [1] HWID        ^|  Windows           ^|   Permanentny
echo:             [2] KMS38       ^|  Windows           ^|    Rok 2038
echo:             [3] Online KMS  ^|  Windows / Office  ^|    180 Dní
echo:             __________________________________________________      
echo:
echo:             [4] Aktivacny Status
echo:             [5] Extra
echo:             [6] Pomoc
echo:             [0] Vypnut
echo:                       Translate By github.com/TopKericek
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "          " %_Red% "Zadajte v klavesnici ponuku [1,2,3,4,5,6,0] :"
choice /C:1234560 /N
set _erl=%errorlevel%
...
echo:       ______________________________________________________________
echo:
echo:             [1] Riesenie aktivacnych problemou
echo:             [2] Zmenit Windows Edíciu
echo:
echo:             [3] Výažok $OEM$ súbor
echo:             [4] Vlozit k¾uc Windows HWID
echo:             [5] Aktivacny Status [vbs]
echo:             __________________________________________________      
echo:                                                                     
echo:             [0] Naspat do menu
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "           " %_Green% "Napíš cislo ktore chcec [1,2,3,4,5,0] :"
choice /C:123450 /N
set _erl=%errorlevel%
...
%eline%
echo Poloha plochy nebola zistená, prerušenie..
echo _____________________________________________________
echo:
call :_color %_Yellow% "Stlac niake tlacitko aby si isel do menu :D"
pause >nul
goto Extras
)
...
%eline%
echo: $OEM$zložka už existuje na ploche.
echo _____________________________________________________
echo:
call :_color %_Yellow% "Stlac niake tlacitko aby si isel do menu..."
pause >nul
goto Extras
)
...
echo:
echo:
echo:                    Extract slozku na pocitac
echo:       ______________________________________________________________
echo:                                                            
echo:             [1] HWID
echo:             [2] KMS38
...
echo:       ______________________________________________________________
echo:  
call :_color2 %_White% "           " %_Green% "Vyber si :):"
choice /C:123450 /N
set _erl=%errorlevel%
...
echo:
call :_color %_Yellow% Stlaè Random tlacitko aby si sa dostal back 
pause >nul
goto Extras
...
echo Projekt je podporovany pre Windows 10/11
goto dk_done
)

if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*Edition~*.mum" (
%eline%
echo HWID Aktivacia neni podporovany pre Windows Servery.
echo Pouzi KMS38 alebo KMS aktivaciu .
goto dk_done
)
...
echo Tento produkt sa nepodarilo nájs v zozname podporovaných produktov.
echo Uistite sa, že používate aktualizovanú verziu skriptu.
echo github.com/TopKericek
goto dk_done
)
...
if !errorlevel! EQU 0 (
echo Mení sa Windows Region na USA         [Uspesne]
) else (
call :dk_color %Red% "Mení sa Windows Region na USA          [Nepodarilo sa]"
)
)
...
call :dk_color %Red% "Generating GenuineTicket.xml            [Nepodarilo sa]"
echo [%encoded%]
if exist "%tdir%\Genuine*" del /f /q "%tdir%\Genuine*" %nul%
goto :dl_final
) else (
echo Generuje sa GenuineTicket.xml          [Uspesne]
)
...
%_xmlexist% (
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "Inštalácia GenuineTicket.xml       [Zlyhalo reštartovanie služby ClipSVC, poèkajte...]"
)
)
...
call :dk_checkperm
if defined _perm (
call :dk_color %Green% "%winos% Je Navzdy aktivovany"
goto :dl_final
)
...
call :dk_color %Red% "Odstranenie Registra                    [Neuspesne] [!_ident!]"
) || (
echo Odstranenie registra                                     [uspesne] [!_ident!]
)
...
if !errorlevel! EQU 0 (
echo Vraca sa windows Region                [Failed] [%name%-%nation%]"
)
)
...
call :dk_color %Red% "Povolenie zakazanych sluzieb            [Neuspesne] [%serv_cste%] [Restart System]"
) || (
call :dk_color %Red% "Povolenie zakázaných služieb            [Neuspesne] [%serv_cste%]"
)
)
...
call :dk_color %Gray% "Hladam WMIC.exe                       [Nenasiel som]"
)


%psc% $ExecutionContext.SessionState.LanguageMode 2>nul | find /i "Full" 1>nul || (
set error=1
call :dk_color %Red% "Kukam Powershell                     [Nereaguje]"
)
...
if %errorlevel% NEQ 0 (
set error=1
call :dk_color %Red% "Checking WMI                            [Nereaguje] %_wmic%"
)
...
echo:
echo                 [1] KMS38 Aktivacia
echo                 ____________________________________________
echo:
echo                 [2] Vymazat KMS38 ochanu
echo:
echo                 [0] %_exitmsg%
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Red% "Enter a menu option in the Keyboard [1,2,0]"
choice /C:120 /N
set _el=!errorlevel!
if !_el!==3  exit /b
...
echo ___________________________________________________________________________________________
echo:
call :dk_color2 %_White% "     " %Green% "Checking: %winos% Je plne aktivovana."
call :dk_color2 %_White% "     " %Gray% "Activacia neni potrebna :)."
echo ___________________________________________________________________________________________
if %_unattended%==1 goto dk_done
echo:
choice /C:10 /N /M ">    [1] Aktivovat [0] %_exitmsg% : "
if errorlevel 2 exit /b
)
cls
...
%nceline%
echo wmic.exe Nereaguje v systeme.
...
set "_title=Online KMS Aktivacia"
set _gui=
...
echo.       ______________________________________________________________
echo.
echo.              [1] Aktivovat - Windows
echo.              [2] Aktivovat - Office
echo.              [3] Aktivovat - All
echo.
...
echo.
if %_Debug%==0 (
echo.              [6] Zapnut  Debug Mode         [Nie]
) else (
call :_color2 %_White% "              [6] Enable Debug Mode         " %_Red% "[Ano]"
)
if %vNextOverride% EQU 1 (
if %sub_next% EQU 1 (
call :_color2 %_White% "              [7] Override Office vNext     " %_Red% "[Ano]"
) else (
echo               [7] Override Office vNext     [Ano]
)
) else (
if %sub_next% EQU 1 (
call :_color2 %_White% "              [7] Override Office vNext     " %_Yellow% "[Nie]"
) else (
echo               [7] Override Office vNext     [Nie]
)
)
